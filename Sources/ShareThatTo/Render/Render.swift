    ...
    func renderThumbnailAndVideo(videoURL: URL, completion: @escaping (Result<(Data, Data), Swift.Error>) -> Void)
    {
        ...
        ThumbnailCreator.thumbnail(videoURL: videoURL) { (result) in
            ...
            case .failure(let error):
                Logger.shareThatToDebug(string: "[ThumbnailCreator] failed to create thumbnail", error: error)
        ...
        VideoExporter.exportVideo(videoURL: videoURL) { (result) in
            ...
            {
            case .failure(let error):
                Logger.shareThatToDebug(string: "[VideoExporter] failed to create video export", error: error)
            case .success(let data):
                renderedVideo = data
            }
            dispatchGroup.leave()
